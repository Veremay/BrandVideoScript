...
3. **只产 position**（把观众视角表达为明确立场）；**不要产 issue**。`source_type` 限：`audience_persona`、`audience_simulation`。position 可独立存在，**不要写 edges**；冲突由 **Coordinator** 后续分析并分配 `conflict_tags`。"""
