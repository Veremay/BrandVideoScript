...
1. 上传 Brief 后应调用后端解析接口。
2. 支持文件类型：PDF、DOC、DOCX、TXT、MD、PPT、PPTX。
3. 解析完成后应触发品牌方 Agent 初始分析。
...
1. Header：品牌方 Agent、状态 badge。
2. Pinned 区：显式需求、隐式需求、品牌反馈三个 tab。
3. Pinned item：文本可编辑、可删除。
4. 新增按钮：添加需求/添加反馈。
5. Chat 区。
6. Quote tag 区。
7. Input + Send。
...
1. 能展示显式需求、隐式需求、品牌反馈。
2. 用户能编辑、新增、删除 pinned insight。
3. 每条 insight 后端都有 reason/evidence/confidence/status。
4. 用户输入 PR feedback 后可触发 insight 更新。
