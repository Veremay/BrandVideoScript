...
```json
{
  ...
  "evidence": [
    {
      "source_type": "brief | script | chat | web | brand_wiki | pr_feedback",
      "quote": "证据原文",
      "row_id": "row_id",
      "column_id": "column_id"
    }
...
```text
固定上下文：brief 摘要、brand_research 归纳与片段、品牌要求（brand_insights）、当前 persona
...
Brief **仅通过文件上传**（`.md` / `.txt`）写入 project。上传后：

1. 保存 `brief.text`，`parse_status=parsed`。
2. 用 8B 生成 `brief.summary`（可选与解析同步）。
3. **自动** 执行 `BrandBriefPipeline`（见 §6.2.1），不依赖用户二次点击。
...
Agent 调用优先使用 `brief.summary`。用户追问具体 brief 原文依据时，再补充相关原文片段。

#### 6.2.1 BrandBriefPipeline（Brief 就绪后自动）

设计详见 `docs/superpowers/specs/2026-05-19-phase-3-brand-agentic-search-design.md`。

```text
brief.text 就绪
  → 识别品牌实体（8B 或规则）
  → Agentic Search
       ├─ llm-wiki：llm-wiki/brands/{brand_slug}/handbook.md 等
       └─ Tavily API：公开品牌/品类信息（可配置关闭）
  → 写入 project.brand_research（snippets + research_summary）
  → Brand Agent 初始分析（32B，结构化 JSON）
...
```text
llm-wiki/brands/{brand_slug}/meta.json
llm-wiki/brands/{brand_slug}/handbook.md
```

**环境变量：** `TAVILY_API_KEY`、`BRAND_WIKI_ROOT`（默认 `./llm-wiki`）。

未配置 Tavily 时降级为 wiki + brief only，并在 `brand_research` 中记录原因。
...
#### 8.1.1 初始分析（Brief 上传后自动）

由 `BrandBriefPipeline` 触发，不经过用户消息。输入：

```text
brief.summary（或 brief.text 截断）
brand_research.research_summary
brand_research.web_snippets（Top-K）
brand_research.wiki_snippets（Top-K）
```

输出：显式/隐式 `brand_insights`（含 evidence：`brief` | `web` | `brand_wiki`）。
